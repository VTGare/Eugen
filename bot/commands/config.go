...
func setup(b *bot.Bot) func(*discordgo.Session, *discordgo.MessageCreate, []string) error {
	return func(s *discordgo.Session, m *discordgo.MessageCreate, args []string) error {
		...
		verifyChannel := func(chID string) bool {
			...
			if err != nil {
				b.Logger().Warn("verifying channel", "err", err)
				return false
			}
			if ch.GuildID != m.GuildID {
				return false
		...
		if !exit {
			...
			if err != nil {
				b.Logger().Warn("replacing guild", "err", err)
			}
		}
		...
		if !exit && err == nil {
			...
			embed.Fields = []*discordgo.MessageEmbedField{{Name: "Reason", Value: reason, Inline: false}}
		}
